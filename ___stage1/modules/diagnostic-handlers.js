...
import { FunctionalHealthVerification } from './functional-health-verification.js';

export class DiagnosticHandlers {
  constructor(diagnosticHelper, vectorStore, dataPersistence, projectManagement, htaCore, taskStrategyCore) {
  ...
  async verifySystemHealth(args = {}) {
    try {
      // Instantiate FunctionalHealthVerification with all required dependencies
      const functionalVerifier = new FunctionalHealthVerification(
      ...
      // Run comprehensive functional health check
      const healthCheck = await functionalVerifier.runFunctionalHealthCheck();
      
      // Generate formatted health report
      const healthReport = functionalVerifier.generateHealthReport(healthCheck);
      
      // Return formatted result consistent with existing interface
      return {
        content: healthReport.content,
        health_status: healthCheck.overall_health.toUpperCase(),
        confidence_score: healthCheck.confidence_score,
        false_positive_risk: healthCheck.false_positive_risk,
        actionable_errors: healthCheck.actionable_errors,
        test_results: healthCheck.test_results,
        tested_functionality: healthCheck.tested_functionality
      };
    } catch (error) {
      return {
        content: [{
          type: 'text',
          text: `**System Health Verification Failed** ❌\n\nError: ${error.message}\n\nThis may indicate a genuine system issue or missing dependencies.`
        }],
        error: error.message,
        health_status: 'ERROR'
      };
